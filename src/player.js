...
export function createPlayer(scene, world, RAPIER, {
  position = [0, 0, 0],
  color = 0x5db4ff,
  team = 'A',
  isHuman = false,
  ...
  const unit = {
    id, team, isHuman, mesh, body, collider, controller,
    alive: true,
    heldArrow: null,
    aim: new THREE.Vector3(0, 0, -1), // facing/launch direction (set by input or AI)
    get colliderHandle() { return collider.handle },
  ...
  function eliminate({ fell = false } = {}) {
    if (!unit.alive) return
    unit.alive = false
    if (unit.heldArrow) { unit.heldArrow.ground(); unit.heldArrow = null } // drop the arrow
