...
import { moveVector, pointerNDC, consumePress, consumeRelease, pointerDown, clearShoot } from './input.js'
...
async function main() {
  ...
  function enterMenu() {
    if (round) { round.dispose(); round = null }
    phase = 'menu'
    renderScore()
    overlay.show({
      title: 'ARRRROW',
  ...
  function spawnRound() {
    ...
    round = createRound(ctx, { enemies: 3, arrowCount: 7, roundNum: match.round, onOver: endRound })
  ...
  function endRound(winner) {
    ...
    renderScore()
    sfx.win()
    combat.push(`Team ${winner} wins round ${match.round}  (${match.wins.A}–${match.wins.B})`, 'win')

    if (match.wins[winner] >= match.needed) { endMatch(winner); return }
  ...
  function scoreLine() {
    return `<span style="color:#5db4ff">You ${match.wins.A}</span>` +
      ` — ` +
      `<span style="color:#ff5d5d">${match.wins.B} Foe</span>`
  }
  function renderScore() {
    if (phase === 'menu') { scoreEl.hidden = true; return }
  ...
  const preview = new THREE.Line(
    previewGeom,
    new THREE.LineDashedMaterial({ color: 0xffd35d, dashSize: 0.45, gapSize: 0.28, transparent: true, opacity: 0.85 }),
  )
  ...
  function weaponUpdate(dt) {
    ...
    if (!h || !h.alive || !h.heldArrow) { hideAim(); charge.cancel(); return }
    raycaster.setFromCamera(pointerNDC(), camera)
    if (!raycaster.ray.intersectPlane(groundPlane, aimTarget)) { hideAim(); return }
  ...
  const debugLines = new THREE.LineSegments(debugGeom, new THREE.LineBasicMaterial({ vertexColors: true }))
  ...
  function chargeBar() {
    ...
    const tag = charge.charging ? (charge.perfect ? '  PERFECT!' : `  ${Math.round(charge.value * 100)}%`) : '  (hold to wind)'
    return `\ncharge [${s}]${tag}`
  }
