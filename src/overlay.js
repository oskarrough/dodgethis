...
export function createOverlay(elId = 'overlay') {
  ...
  function show({ title = '', subtitle = '', lines = [], actions: acts = [] }) {
    ...
    if (acts.length) {
      ...
      for (const a of acts) {
        ...
        b.addEventListener('click', (e) => { e.stopPropagation(); select(a) })
        row.append(b)
      }
      card.append(row)
    }
  ...
  window.addEventListener('keydown', (e) => {
    if (el.hidden) return
    if (e.code === 'Enter' && actions[0]) { e.preventDefault(); select(actions[0]); return }
    const a = actions.find((x) => x.key === e.code)
    if (a) { e.preventDefault(); select(a) }
  })

  return { show, hide, get visible() { return !el.hidden } }
}
